...
from mmengine.utils import is_seq_of, to_2tuple
...
@MODELS.register_module()
class BatchSyncRandomChoiceResize(nn.Module):
    """Batch random resize which synchronizes the random size across ranks.
    ...
    def __init__(self,
        ...
        super().__init__()
        assert is_seq_of(scales, (tuple, int))
        if is_seq_of(scales, int):
            self.scales = [to_2tuple(s) for s in scales]
        else:
            self.scales = scales
