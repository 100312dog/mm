...
def delta2bbox(rois: Tensor,
    ...
    if clip_border and max_shape is not None:
        bboxes[..., 0::2].clamp_(min=0, max=max_shape[1] - 1)
        bboxes[..., 1::2].clamp_(min=0, max=max_shape[0] - 1)
...
def onnx_delta2bbox(rois: Tensor,
    ...
    if clip_border and max_shape is not None:
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            x1, y1, x2, y2 = dynamic_clip_for_onnx(x1, y1, x2, y2, max_shape - 1)
            bboxes = torch.stack([x1, y1, x2, y2], dim=-1).view(deltas.size())
            return bboxes
        if not isinstance(max_shape, torch.Tensor):
            max_shape = x1.new_tensor(max_shape) - 1
